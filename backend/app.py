...
# Optional assets (not bundled by default)
...
def analyze_social_engineering(password, personal_info):
    ...
    for key, value in components.items():
        ...
        if value in pwd_lower or val_norm in pwd_norm:
            ...
            unmatched_chars = list(pattern.sub('', ''.join(unmatched_chars)))
            found_any = True
        else:
    ...
    unmatched_percentage = round((len(unmatched_chars) / len(password)) * 100, 2) if password else 0.0
    return unmatched_percentage, analysis_report, found_any
...
def analyze_password():
    ...
    unmatched_percentage, se_report, used_personal = analyze_social_engineering(password, personal_info)
    score = base_score
